...
const int wdtTimeout = 3000; //ms
hw_timer_t *timer = NULL;

const int delay_time = 5000;//ms

const int heater_wait_timeout_min = 30;//min
const int heater_active_timeout_min = 2;//min
...
void FSM_sensors(Adafruit_SHT31& sht_obj,float sht_data, float lux_data,const void* pBuf,uint8_t* msg,osjob_t* sendjob ){
  
  static sensors_states sen_st = INI_WDT;
  unsigned long int heater_time;
  unsigned long int prev_time;
  //hw_timer_t *wdt_timer = NULL; // PASAR COMO REFERENCIA PARA INICIALIZAR
  ...
  while(1)
  {
    switch(sen_st){
      ...
      case KTD_3:
        ...
        break;
      case SEND_DATA:
        //Poner concatenacion para mensaje y actualizacion de msg
        msg= new_msg;
...
void loop() {
    ...
    FSM_sensors(sht31,sht_data,lux_data,buf,mydata,&sendjob);
}
