...
float ph_acid = 4.0;
float acid_voltage = 2850;
float ph_neutral = 6.8;
float neutral_voltage = 2100;
...
float PH_read(float ph_acid,float acid_voltage, float ph_neutral, float neutral_voltage){
  ...
  if(millis()-timepoint>3000){
    ...
    slope = (ph_acid - ph_neutral)/((neutral_voltage-1500)/3.0 - (acid_voltage-1500)/3.0);
  ...
  while(1)
  {
    switch(sen_st){
      ...
      case PH_READ:
        ...
        ph_data = PH_read(ph_acid,acid_voltage,ph_neutral,neutral_voltage);
        sen_st = KTD_3;
        break;
      case KTD_3:
      ...
      case SEND_DATA:
        ...
        msg = "Temperatura: " + String(sht_vec[0]) + "," + "Humedad: " + String(sht_vec[1]) + "," + "Luminocidad. " + String(lux_val);
        Serial.println(msg);
        sen_st = KTD_4;
        break;
      case KTD_4:
...
void FSM_sensors_prueba_2(Adafruit_SHT31& sht_obj,hw_timer_t* wdt_timer){
  static sensors_states sen_st = INI_WDT;
