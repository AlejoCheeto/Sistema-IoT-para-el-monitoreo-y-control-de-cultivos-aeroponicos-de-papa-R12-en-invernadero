...
const int c = 90; // Number or readings after compute an averaged sample

//DELAY TIME
const int delay_time = 300;    // Delay time between each data measurment (ms)
const int delay_time_send = 30000;
...
float acidVoltage = 1850;
float neutralVoltage = 1350;
  ...
  switch(c_st){
    ...
    break;
     case KTD_2:
      timerWrite(wdt_timer,0);
      if(loraflag == 0){
        if((prev_time - millis())< delay_time_send){
          c_st = DELAY;
        } else if ((millis() -prev_time) > delay_time_send){
          c_st = KTD_3;
        }
      } else{
           c_st = KTD_3;
